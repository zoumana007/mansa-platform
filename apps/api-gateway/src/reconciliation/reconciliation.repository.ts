...
function encodeCursor(createdAt: Date, id: string): string {
  return Buffer.from(
    JSON.stringify({ createdAt: createdAt.toISOString(), id } satisfies CursorPayload),
  ).toString('base64url');
}
...
export class ReconciliationRepository {
          ...
          await tx.reconciliationItem.createMany({
            data: input.items.map((item) => ({
              ...
              ...(item.providerReference
                ? { providerReference: item.providerReference.trim() }
                : {}),
              ...(item.internalAmountMinor === undefined
                ? {}
                : { internalAmountMinor: item.internalAmountMinor }),
              ...(item.providerAmountMinor === undefined
                ? {}
                : { providerAmountMinor: item.providerAmountMinor }),
              currency: item.currency.trim().toUpperCase(),
              ...(item.internalStatus
                ? { internalStatus: item.internalStatus.trim().toUpperCase() }
                : {}),
              ...(item.providerStatus
                ? { providerStatus: item.providerStatus.trim().toUpperCase() }
                : {}),
              ...
              ...(item.rawLineFingerprint
                ? { rawLineFingerprint: item.rawLineFingerprint.trim() }
                : {}),
            })),
          });
        }
  ...
  public async resolveItem(input: ResolveReconciliationItemInput) {
    ...
    if (!resolutionNote || !reasonCode || !idempotencyKey || !correlationId || !actorId || !actorType) {
      throw new Error(
        'resolutionNote, reasonCode, idempotencyKey, correlationId, actorId and actorType are required',
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (replay) {
        if (replay.id !== input.itemId || replay.status !== input.status) {
          throw new Error('idempotency key already used for a different resolution');
        }
        return replay;
      }
