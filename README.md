...
L’observabilité process-local du rapprochement est désormais instrumentée. `ReconciliationOperationalMonitor` suit succès, échecs, volume importé, durée monotone cumulée et dernière durée. Il agrège aussi les résultats métier `MATCHED` / `MISMATCHED` ainsi que les motifs d’écart issus de l’énumération fermée du domaine. `LowCardinalityReconciliationMetricsExporter` expose uniquement des métriques à noms fixes, sans labels tenant, fournisseur, batch, transaction ou fichier.

La prochaine tranche de rapprochement doit définir un point d’exposition interne des métriques avec authentification dédiée, afin qu’un workload tenant-scopé ne puisse pas observer l’activité agrégée d’autres organisations. Ensuite pourront venir des alertes/SLI/SLO et des adaptateurs partenaires derrière un contrat strict, sans inventer de schéma fournisseur avant disponibilité d’un format réel ou d’une sandbox officielle.

Les spécifications correspondantes se trouvent dans `mansa-docs/volume-08-donnees-analytics/10-moteur-rapprochement-financier.md`, `mansa-docs/volume-08-donnees-analytics/11-validation-postgresql-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/12-isolation-tenant-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/13-filtres-et-contrats-http-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/14-observabilite-operationnelle-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/15-contrat-export-metriques-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/16-duree-imports-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/17-agregats-resultats-metier-rapprochement.md` et `mansa-docs/volume-01-socle-technique/15-identite-et-autorisation-workload.md`.
