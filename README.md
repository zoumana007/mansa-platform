...
`WorkloadIdentityVerifier` isole la vérification cryptographique brute du mécanisme choisi ultérieurement (OIDC/JWKS, mTLS/SPIFFE ou équivalent approuvé). `WorkloadIdentityGuard` extrait le bearer credential, délègue la vérification et n’attache à la requête que le contexte normalisé. `WorkloadScopeGuard` et `RequireWorkloadScopes(...)` appliquent ensuite l’autorisation de façon fail-closed : une route sans politique déclarée est refusée, comme une requête sans contexte ou avec un scope manquant.

Cette infrastructure n’est pas encore activée sur toutes les routes internes. Le prochain lot doit fournir le verifier de production, l’enregistrer dans NestJS, migrer les contrôleurs concernés et supprimer progressivement les `organizationId` fournis librement par l’appelant.
...
Les filtres prévus par le contrat partagé sont maintenant appliqués au niveau repository : fournisseur, statut et période pour les lots ; fournisseur, statut, motif d’écart, références et fenêtre de création pour les items. Ils restent cumulables avec la pagination keyset et l’isolation organisationnelle.

Le contrôleur interne exige toujours un `organizationId` explicite sur toutes les routes de rapprochement. Cette exigence reste transitoire derrière `InternalServiceGuard`. L’infrastructure d’identité workload et de scopes existe désormais, mais elle n’est pas encore câblée à ce contrôleur ; la migration doit utiliser `request.workloadIdentity.organizationId` et des scopes `reconciliation:read` / `reconciliation:write`.

Les réponses HTTP ne renvoient plus directement les modèles Prisma. `reconciliation.presenter.ts` sérialise les lots vers `ReconciliationBatchSummary` et les items vers `ReconciliationItem`, convertit les dates en ISO 8601, vérifie les montants `bigint` avant conversion et exclut les champs internes tels que `organizationId`, empreintes de source/ligne, métadonnées et clés d’idempotence de résolution.

Les tests de contrôleur couvrent l’absence de portée, la propagation de l’organisation, les filtres, la validation des enums/dates, les comportements `404/400` et l’absence de fuite des champs Prisma internes. La recette PostgreSQL couvre l’isolation lecture/liste, l’idempotence d’import par tenant, la coexistence d’une même source dans deux organisations, la pagination scoppée et le refus d’une résolution inter-tenant.

La prochaine tranche doit câbler l’identité workload attestée au rapprochement, puis ajouter métriques/alertes et premiers adaptateurs partenaires réels.

Les spécifications correspondantes se trouvent dans `mansa-docs/volume-08-donnees-analytics/10-moteur-rapprochement-financier.md`, `mansa-docs/volume-08-donnees-analytics/11-validation-postgresql-rapprochement.md`, `mansa-docs/volume-08-donnees-analytics/12-isolation-tenant-rapprochement.md` et `mansa-docs/volume-08-donnees-analytics/13-filtres-et-contrats-http-rapprochement.md`.
